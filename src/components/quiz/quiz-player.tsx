    ...
    return (
      <main className="mx-auto w-full min-w-0 max-w-2xl px-4 py-6 pb-[max(3rem,env(safe-area-inset-bottom))] sm:px-6 sm:py-12">
        <div className="rounded-2xl border bg-card p-5 text-center shadow-sm sm:p-8">
          ...
          <h2 className="text-xl font-semibold tracking-tight sm:text-2xl">Quiz complete</h2>
          ...
          <div className="mt-6 space-y-2.5 sm:mt-8">
            <div className="flex flex-col gap-2.5 sm:flex-row">
              <Button
                variant="outline"
                className="h-11 w-full touch-manipulation sm:h-10 sm:flex-1"
                onClick={handleRetry}
            ...
            <ul className="space-y-4 sm:space-y-5">
              {misses.map(({ question, selectedAnswer, number }) => {
                ...
                return (
                  <li
                    key={question.id}
                    className="min-w-0 rounded-2xl border border-border/60 bg-card/80 shadow-sm backdrop-blur-sm"
                  >
                    <div className="border-b border-border/50 px-4 py-4 sm:px-6 sm:py-5">
                      <div className="flex flex-wrap items-center gap-2.5">
                        <span className="bg-muted text-muted-foreground inline-flex size-7 shrink-0 items-center justify-center rounded-lg text-[0.6875rem] font-semibold tabular-nums">
                          {String(number).padStart(2, "0")}
                        </span>
                        {!category && question.category && (
                          <span className="text-muted-foreground min-w-0 text-[0.6875rem] font-medium tracking-wider uppercase">
                            {question.category}
                          </span>
                        )}
                      </div>
                      <p className="mt-3 text-[0.9375rem] font-semibold tracking-tight text-balance break-words sm:text-lg sm:leading-snug">
                        {question.question}
                      </p>
                    </div>

                    <div className="space-y-2.5 px-4 py-4 sm:px-6 sm:py-5">
                      <div className="flex min-w-0 items-start gap-3 rounded-xl border border-destructive/20 bg-destructive/[0.04] px-3 py-3 sm:px-3.5">
                        <XCircle className="text-destructive mt-0.5 size-4 shrink-0" />
                        <div className="min-w-0 flex-1">
                          <p className="text-destructive/80 text-[0.6875rem] font-medium tracking-wider uppercase">
                            Your answer
                          </p>
                          <p className="text-destructive mt-1 text-sm leading-relaxed break-words">
                            {yourAnswer}
                          </p>
                        </div>
                      </div>

                      <div className="flex min-w-0 items-start gap-3 rounded-xl border border-emerald-500/25 bg-emerald-500/[0.05] px-3 py-3 sm:px-3.5">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="mt-1 text-sm leading-relaxed break-words text-emerald-800 dark:text-emerald-300">
                            {correct}
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="space-y-3 border-t border-border/50 px-4 py-4 sm:space-y-4 sm:px-6 sm:py-5">
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="group text-foreground inline-flex max-w-full min-w-0 items-start gap-1.5 text-sm font-medium transition-opacity hover:opacity-70"
                      >
                        ...
                        <ArrowUpRight
                          aria-hidden
                          className="text-muted-foreground mt-0.5 size-3.5 shrink-0 transition-transform group-hover:-translate-y-px group-hover:translate-x-px"
                        />
                      </a>
                    </div>
                  </li>
